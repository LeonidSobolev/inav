...
F7_TARGETS      = $(F7X2RE_TARGETS) $(F7X5XE_TARGETS) $(F7X5XG_TARGETS) $(F7X5XI_TARGETS) $(F7X6XG_TARGETS) $(F7X7ZI_TARGETS)
H7_TARGETS      = $(H753_TARGETS)
...
ifeq ($(filter $(TARGET),$(F1_TARGETS) $(F3_TARGETS) $(F4_TARGETS) $(F7_TARGETS) $(H7_TARGETS) ),)
$(error Target '$(TARGET)' has not specified a valid STM group, must be one of F1, F3, F405, F411, F427 or F7x pr H7(FiXaR). Have you prepared a valid target.mk?)
endif
...
2048K_TARGETS = $(F427_TARGETS) $(F7X5XI_TARGETS) $(H7_TARGETS) $(F7X7ZI_TARGETS)
...
GROUP_3_TARGETS := OMNIBUS AIRBOTF4 BLUEJAYF4 OMNIBUSF4 OMNIBUSF4PRO OMNIBUSF4V3 SPARKY2 MATEKF405 MATEKF405SE OMNIBUSF7 DYSF4PRO MATEKF405OSD MATEKF405_SERVOS6
...
ifeq ($(TARGET),$(filter $(TARGET),$(F7X5XG_TARGETS) $(H7_TARGETS)))
...
else ifeq ($(TARGET),$(filter $(TARGET), $(F7X7ZI_TARGETS)))
DEVICE_FLAGS    = -DSTM32F746xx -DUSE_HAL_DRIVER -D__FPU_PRESENT
LD_SCRIPT       = $(LINKER_DIR)/stm32_flash_f767_fixar.ld
STARTUP_SRC     = startup_stm32f746xx.s
...
#START H7 TARGETS (FIXAR)
else ifeq ($(TARGET),$(filter $(TARGET), $(H7_TARGETS)))

#STDPERIPH
STDPERIPH_DIR   = $(ROOT)/lib/main/STM32H7xx_HAL_Driver
STDPERIPH_SRC   = $(notdir $(wildcard $(STDPERIPH_DIR)/Src/*.c))
...
#CMSIS
VPATH           := $(VPATH):$(CMSIS_DIR)/CM7/Include:$(CMSIS_DIR)/CM7/Device/ST/STM32H7xx
VPATH           := $(VPATH):$(STDPERIPH_DIR)/Src
CMSIS_SRC       = $(notdir $(wildcard $(CMSIS_DIR)/CM7/Include/*.c \
                  $(CMSIS_DIR)/CM7/Device/ST/STM32H7xx/*.c))
INCLUDE_DIRS    := $(INCLUDE_DIRS) \
                   $(STDPERIPH_DIR)/Inc \
                   $(USBCORE_DIR)/Inc \
                   $(USBCDC_DIR)/Inc \
                   $(CMSIS_DIR)/CM7/Include \
                   $(CMSIS_DIR)/CM7/Device/ST/STM32H7xx/Include \
                   $(ROOT)/src/main/vcp_hal
...
DEVICE_FLAGS    = -DSTM32H753xx -DUSE_HAL_DRIVER -D__FPU_PRESENT
LD_SCRIPT       = $(LINKER_DIR)/STM32H753ZITX_FLASH.ld
STARTUP_SRC     = startup_stm32h753zitx.s
...
VCP_SRC = \
            ...
            drivers/usb_io.c
else ifeq ($(TARGET),$(filter $(TARGET),$(F7_TARGETS) $(H7_TARGETS)))
VCP_SRC = \
            vcp_hal/usbd_desc.c \
            vcp_hal/usbd_conf.c \
...
TARGET_SRC := $(STARTUP_SRC) $(STM32H7xx_COMMON_SRC) $(TARGET_SRC)
else ifeq ($(TARGET),$(filter $(TARGET),$(F3_TARGETS)))
...
ifeq ($(TARGET),$(filter $(TARGET),$(F7_TARGETS) $(F4_TARGETS) $(F3_TARGETS) $(H7_TARGETS)))
...
#excludes
ifeq ($(TARGET),$(filter $(TARGET),$(F7_TARGETS) $(H7_TARGETS)))
...
%generated.h %generated.c:
	...
	$(V1) echo "CFLAGS=\"$(CFLAGS)\" TARGET=$(TARGET) ruby $(SETTINGS_GENERATOR) . $(SETTINGS_FILE)"
...
$(OBJECT_DIR)/$(TARGET)/%.o: %.c
	$(V1) mkdir -p $(dir $@)
	$(V1) echo %C% $(notdir $<) "$(STDOUT)"
	$(V1) echo "$(CROSS_CC) -c -o $@ $(CFLAGS) $<"
	$(V1) $(CROSS_CC) -c -o $@ $(CFLAGS) $<

# Assemble
$(OBJECT_DIR)/$(TARGET)/%.o: %.s
	$(V1) mkdir -p $(dir $@)
	$(V1) echo %s% $(notdir $<) "$(STDOUT)"
