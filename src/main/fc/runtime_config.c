...
uint32_t enableFlightMode(flightModeFlags_e mask)
{
    ...
    if (flightModeFlags != oldVal){
        ...
        cur_Flags.INAV_flags.flightModeFlags = flightModeFlags;
...
// FIXAR LOGGER improvement: enable/disable state with writing into log

uint32_t enableState(stateFlags_t mask)
{
    uint32_t oldVal = stateFlags;

    stateFlags |= (mask);
    if (stateFlags != oldVal){
#if defined(USE_TELEMETRY_XLOGGER)
        cur_Flags.INAV_flags.stateFlags = stateFlags;
        cur_Flags.INAV_flags.curTime = millis();
        cur_Flags.INAV_flags.newData = 1;
#endif
    }
...
uint32_t disableState(stateFlags_t mask)
{
    uint32_t oldVal = stateFlags;
