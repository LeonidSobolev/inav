...
static const box_t boxes[CHECKBOX_ITEM_COUNT + 1] = {
    ...
    { BOXSWITCHTOPLANE, "SWITCH TO PLANE", 42 },
...
void initActiveBoxIds(void)
{
    ...
    activeBoxIds[activeBoxIdCount++] = BOXSWITCHTOPLANE;
...
// #endif
    ...
    // if (!feature(FEATURE_FAILOFF)) {
        // activeBoxIds[activeBoxIdCount++] = BOXFAILOFF;
    // }
...
// #ifdef USE_GPS
    ...
    // if ((feature(FEATURE_GPS) && sensors(SENSOR_ACC)) || (STATE(FIXED_WING) && sensors(SENSOR_ACC) && feature(FEATURE_GPS))) {
        // activeBoxIds[activeBoxIdCount++] = BOXNAVPOSHOLD;
        // activeBoxIds[activeBoxIdCount++] = BOXNAVRTH;
        // activeBoxIds[activeBoxIdCount++] = BOXNAVWP;
...
void packBoxModeFlags(boxBitmask_t * mspBoxModeFlags)
{
    ...
    CHECK_ACTIVE_BOX(IS_ENABLED(IS_RC_MODE_ACTIVE(BOXFAILOFF)),     BOXFAILOFF);
    ...
    CHECK_ACTIVE_BOX(IS_ENABLED(IS_RC_MODE_ACTIVE(BOXSWITCHTOPLANE)),     BOXSWITCHTOPLANE);
