...
throttleStatus_e calculateThrottleStatusRcCommand(void)
{
    const uint16_t deadband3d_throttle = rcControlsConfig()->deadband3d_throttle;
    if (feature(FEATURE_3D) && (rcCommand[THROTTLE] > (rxConfig()->midrc - deadband3d_throttle) && rcCommand[THROTTLE] < (rxConfig()->midrc + deadband3d_throttle)))
        return THROTTLE_LOW;
    else if (!feature(FEATURE_3D) && (rcCommand[THROTTLE] < rxConfig()->mincheck))
        return THROTTLE_LOW;
...
void processRcStickPositions(throttleStatus_e throttleStatus, bool disarm_kill_switch, bool fixed_wing_auto_arm)
{
    ...
    if (!isUsingSticksForArming()) {
        if (IS_RC_MODE_ACTIVE(BOXARM)) {
            ...
            //CX: Disarm only in MANUAL mode
            if (IS_RC_MODE_ACTIVE(BOXMANUAL) && ARMING_FLAG(ARMED) && !IS_RC_MODE_ACTIVE(BOXFAILSAFE) && rxIsReceivingSignal() && !failsafeIsActive()) {
                rcDisarmTicks++;
