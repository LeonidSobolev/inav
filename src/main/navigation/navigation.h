...
typedef struct navConfig_s {
    ...
    struct {
        ...
        uint16_t autostart_throttle; 			// minimum throttle for autostart
        uint16_t autostart_update_time; 		// autostart baro update cycle
        uint16_t autostart_throttle_adds; 		// throttle coefficient autostart
		uint16_t autostart_vertical_speed;		// max vertical autostart speed
		uint16_t autostart_timeout;				// timeout to reach first WP altitude
		bool autostart_baro_only;
		bool autoland_stage_1;
		bool autoland_stage_2;        
    } general;
...
extern double speed_brake_gain;
