...
float initialBaroAltitudeOffset = 0.0f;

void reset_baro_alt(){
    initialBaroAltitudeOffset = baroGetLatestAltitude();
}
/**
...
void updatePositionEstimator_BaroTopic(timeUs_t currentTimeUs)
{
    ...
    float newBaroAlt = baroCalculateAltitude(currentTimeUs);
...
static void publishEstimatedTopic(timeUs_t currentTimeUs)
{
    ...
    // if(systemConfig()->cx_use_gps_az > 0){
        // float az_val = sqrt( posEstimator.est.vel.x*posEstimator.est.vel.x + posEstimator.est.vel.y*posEstimator.est.vel.y);
        // if( az_val > 0.1 ){
          // float az = atan2( posEstimator.est.vel.y, posEstimator.est.vel.x );
          // updateActualHeading(true, az * 180.0 / 3.1415 * 100);
        // } else {
...
void updatePositionEstimator(void)
{
    ...
    //ICS Position store
    if( (gpsSol.fixType == GPS_NO_FIX) || (is_gps_off) ){
    	cur_pos.az = attitude.values.yaw;
    	cur_pos.gndspd = 0;
    	cur_pos.gps_ok = 0;
    } else {
		cur_pos.la = gpsSol.llh.lat;
		cur_pos.lo = gpsSol.llh.lon;
	    if(systemConfig()->cx_use_gps_az == 0){
	        cur_pos.az = attitude.values.yaw;
	    } else {
	        cur_pos.az = gpsSol.groundCourse;
	    }

	    cur_pos.gps_ok = 1;
    }

	cur_pos.h = getEstimatedActualPosition(Z);

	cur_pos.inav_x_pos = getEstimatedActualPosition(X);
	cur_pos.inav_y_pos = getEstimatedActualPosition(Y);


    cur_pos.mag_az =  attitude.values.yaw;
        
    if (sensors(SENSOR_PITOT)) {
        cur_pos.airspd = pitot.airSpeed;
        
        if( cur_pos.airspd < 5000 ){
            cur_pos.is_use_airspeed = 1;
        }
    } else {
        cur_pos.airspd = 0;
        cur_pos.is_use_airspeed = 0;
    }
    
    if( systemConfig()->cx_use_baro_or_filter_altspd == 0 ){
    	cur_pos.hspd = baroGetLatestAltSpeed();
    } else {
    	cur_pos.hspd = getEstimatedActualVelocity(Z);
    }
    cur_pos.gndspd = sqrtf(sq(getEstimatedActualVelocity(X)) + sq(getEstimatedActualVelocity(Y)));
