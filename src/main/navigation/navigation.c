...
timeMs_t landing_pause_start_time = 0;
double speed_brake_gain;
...
static const navigationFSMStateDescriptor_t navFSM[NAV_STATE_COUNT] = {
    ...
    [NAV_STATE_RTH_LANDING] = {
        ...
        .timeoutMs = 10,
        //NAV_CTL_ALT | NAV_REQUIRE_ANGLE_FW | NAV_REQUIRE_THRTILT | NAV_RC_ALT
        ...
        //.stateFlags = NAV_CTL_ALT |  NAV_CTL_LAND | NAV_REQUIRE_ANGLE | NAV_REQUIRE_MAGHOLD | NAV_REQUIRE_THRTILT | NAV_AUTO_RTH ,
        .mapToFlightModes = NAV_RTH_MODE | NAV_ALTHOLD_MODE,
    ...
    [NAV_STATE_LAUNCH_INITIALIZE] = {
        ...
        .mwError = MW_NAV_ERROR_NONE,
        .onEvent = {
            [NAV_FSM_EVENT_TIMEOUT]                     = NAV_STATE_LAUNCH_INITIALIZE,
    ...
    [NAV_STATE_LAUNCH_IN_PROGRESS] = {
        ...
        .onEvent = {
            ...
            [NAV_FSM_EVENT_SUCCESS]                     = NAV_STATE_WAYPOINT_INITIALIZE,
...
//CX: Defines in pid.h :)
uint8_t cxIsIPlane(){  
    if( systemConfig()->cx_use_plane_mode == 0) return 0;
    if(posControl.activeWaypointIndex < (posControl.waypointCount - 2) ){
        return 1;
    }
    return 0;
}
...
static navigationFSMEvent_t navOnEnteringState_NAV_STATE_RTH_INITIALIZE(navigationFSMState_t previousState)
{
    ...
    if (STATE(FIXED_WING) && (posControl.homeDistance < navConfig()->general.min_rth_distance) && !posControl.flags.forcedRTHActivated) {
        ...
        systemConfigMutable()->cx_debug = 1; 
    ...
    if ((posControl.flags.estPosStatue >= EST_USABLE) || navConfig()->general.flags.rth_climb_ignore_emerg) {
        ...
        if (posControl.homeDistance < navConfig()->general.min_rth_distance) {
            ...
            landing_pause_start_time = millis();
...
static navigationFSMEvent_t navOnEnteringState_NAV_STATE_RTH_HOVER_PRIOR_TO_LANDING(navigationFSMState_t previousState)
{
    ...
    if ((posControl.flags.estPosStatue >= EST_USABLE) || !checkForPositionSensorTimeout()) {
        ...
        if (STATE(FIXED_WING)) {
            ...
            //cxnav: I think it is our landing trouble. Condition deleted
            //if (ABS(wrap_18000(posControl.homeWaypointAbove.yaw - posControl.actualState.yaw)) < DEGREES_TO_CENTIDEGREES(15)) {
            //if( millis() - landing_pause_start_time > systemConfig()->cx_prior_landing_pause_ms ){
            if( millis() - landing_pause_start_time > 1000 ){
                resetLandingDetector();
                return NAV_FSM_EVENT_SUCCESS;
            }            
            else if (!validateRTHSanityChecker()) {
...
static navigationFSMEvent_t navOnEnteringState_NAV_STATE_RTH_LANDING(navigationFSMState_t previousState)
{
    ...
    // if(systemConfig()->cx_landing_poshold_enable > 0){
        // return NAV_FSM_EVENT_SWITCH_TO_EMERGENCY_LANDING;
    // }
    ...
    if (!ARMING_FLAG(ARMED)) {
        ...
        // if (navigationRTHAllowsLanding()) {
            ...
            // if ((posControl.flags.estSurfaceStatus == EST_TRUSTED) && posControl.actualState.surface < 50.0f) {
                ...
                // descentVelLimited = MIN(-0.15f * navConfig()->general.land_descent_rate, -10.0f);
            // }
            // else {
                ...
                // descentVelLimited = MIN(-descentVelScaling * navConfig()->general.land_descent_rate, -10.0f);
            // }
...
static navigationFSMEvent_t navOnEnteringState_NAV_STATE_WAYPOINT_IN_PROGRESS(navigationFSMState_t previousState)
{
    ...
    if ((posControl.flags.estPosStatue >= EST_USABLE) && (posControl.flags.estHeadingStatus >= EST_USABLE)) {
        ...
        // Brake activating
        double sg = 1;       
        if( systemConfig()->cx_land_brake_enable == 1 ){
            if( systemConfig()->cx_land_brake_distance2_m > 1 ){
                if( posControl.activeWaypointIndex >= (posControl.waypointCount - 2) - 1){
                    uint32_t wpDistance = calculateDistanceToDestination(&posControl.activeWaypoint.pos);
                    sg = wpDistance / systemConfig()->cx_land_brake_distance2_m;
                    if( sg > 1 ) sg = 1;
                    if( sg < 0 ) sg = 0;
                }  
            }
    ...
    if( STATE(FIXED_WING) ){
        ...
        if(feature(FEATURE_FW_LAUNCH)){
            ...
            if ((throttleStatus == THROTTLE_LOW) && (areSticksDeflectedMoreThanPosHoldDeadband())){ 
                systemConfigMutable()->cx_debug = 2;
                return NAV_FSM_EVENT_SWITCH_TO_IDLE;
            }
        }
    } else {
...
static navigationFSMEvent_t navOnEnteringState_NAV_STATE_LAUNCH_IN_PROGRESS(navigationFSMState_t previousState)
{
    ...
    if( STATE(FIXED_WING) ){
        ...
        int dest_alt = systemConfig()->cx_launch_height; // cm        
        
        int current_alt = baroGetLatestAltitude();
        if(dest_alt <= current_alt){         
            return NAV_FSM_EVENT_SUCCESS;  
        }
        return NAV_FSM_EVENT_NONE; 
    }
...
void updateActualHorizontalPositionAndVelocity(bool estimateValid, float newX, float newY, float newVelX, float newVelY)
{
    ...
    if( systemConfig()->cx_emulator_mode == 0 ){
        ...
        posControl.actualState.velXY = gpsSol.groundSpeed;
    }
     ...
     //   if (STATE(FIXED_WING)) {
      ...
      //  }
        ...
        if( IS_RC_MODE_ACTIVE(BOXMANUAL)) {
            ...
            systemConfigMutable()->cx_debug = 4;
            return NAV_FSM_EVENT_SWITCH_TO_IDLE;
        } 
        
        //cx: For all AP modes disable XXXHOLD modes
        if( posControl.navState >= NAV_STATE_RTH_INITIALIZE ) return NAV_FSM_EVENT_NONE;
        ...
        systemConfigMutable()->cx_debug = 6;
        
        //cx: I think, if no happens, doing nothing. Later, switch to IDLE.
        return NAV_FSM_EVENT_NONE;
    }
    else {
...
{
    /* Initial state */
//     systemConfigMutable()->cx_debug = 0; 
