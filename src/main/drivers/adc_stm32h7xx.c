...
static adcDevice_t adcHardware[ADCDEV_COUNT] = {
		...
		{ .ADCx = ADC1, .rccADC = 0, .rccDMA = RCC_AHB1(DMA2), .DMAy_Streamx = DMA2_Stream4, .channel = 0, .enabled = false, .usedChannelCount = 0 },
...
//     AdcHandle.Init.OversamplingMode      = DISABLE;                       /* No oversampling */
    ...
    adc->ADCHandle.Init.ClockPrescaler        = ADC_CLOCK_ASYNC_DIV2;
    ...
    adc->ADCHandle.Init.ExternalTrigConv      = ADC_SOFTWARE_START;
...
void adcHardwareInit(drv_adc_config_t *init)
{
    ...
    for (int i = ADC_CHN_1; i < ADC_CHN_COUNT; i++) {
        ...
        //adcConfig[i].sampleTime = ADC_SAMPLETIME_810CYCLES_5;
        adcConfig[i].enabled = true;

//        adcConfig[i].Rank         = ADC_REGULAR_RANK_1;          /* Rank of sampled channel number ADCx_CHANNEL */
//        adcConfig[i].SamplingTime = ADC_SAMPLETIME_810CYCLES_5;   /* Sampling time (number of clock cycles unit) */
//        adcConfig[i].SingleDiff   = ADC_SINGLE_ENDED;            /* Single-ended input channel */
//        adcConfig[i].OffsetNumber = ADC_OFFSET_NONE;             /* No offset subtraction */
//        adcConfig[i].Offset = 0;                                 /* Parameter discarded because offset correction is disabled */
