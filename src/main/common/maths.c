...
float wrap_3600(float angle_decideg)
{
    if (angle_decideg > 3600)
    	angle_decideg -= 3600;
    if (angle_decideg < 0)
    	angle_decideg += 3600;
...
float scaleRangef_zero(float x, float srcMin, float srcZero, float srcMax, float destMin, float destZero, float destMax) {
	// same as scaleRangef, but with specific zero point
	if (x >= srcZero) {
		return scaleRangef(x, srcZero, srcMax, destZero, destMax);
	}
	else {
		return scaleRangef(x, srcMin, srcZero, destMin, destZero);
	}
}
