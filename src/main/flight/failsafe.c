...
void failsafeReset(void)
{
	...
	//start condition 
	failsafeState.failsafe_off_from_nsu = true;	
	failsafeState.failsafe_from_MSP = false;
}
...
void failsafeActivate(failsafePhase_e newPhase)
{
    ...
    uint8_t xlogger_phase = failsafeState.phase;
    cur_Flags.INAV_flags.failsafePhase = xlogger_phase;
...
{
	// don't say FC that RX in failsafe
	if (isFailsafeOffActive() && !isFaisafeFromMSP()){
		return true;
	}else{
...
void failsafeUpdateState(void)
{
	...
	// don't say FC that RX in failsafe
	if (!isFaisafeFromMSP() && isFailsafeOffActive()){
			return;
	}
...
void failsafeSetOff(bool state){
	failsafeState.failsafe_off_from_nsu = state; 
}

bool isFailsafeOffActive(){
	return failsafeState.failsafe_off_from_nsu;
}

void failsafeMSPSet(bool state){
	failsafeState.failsafe_from_MSP = state; 
}

bool isFaisafeFromMSP(){
	return failsafeState.failsafe_from_MSP;
}
