...
#ifdef USE_BLACKBOX
int32_t axisPID_P[FLIGHT_DYNAMICS_INDEX_COUNT], axisPID_I[FLIGHT_DYNAMICS_INDEX_COUNT], axisPID_D[FLIGHT_DYNAMICS_INDEX_COUNT], axisPID_Setpoint[FLIGHT_DYNAMICS_INDEX_COUNT], axisPID_rate[FLIGHT_DYNAMICS_INDEX_COUNT], axisPID_angleTgt[FLIGHT_DYNAMICS_INDEX_COUNT];
#endif
...
void pidResetErrorAccumulatorAxis(int axis)
{
    // Reset axis integrator
    //for (int axis = 0; axis < 3; axis++) {
        pidState[axis].errorGyroIf = 0.0f;
        pidState[axis].errorGyroIfLimit = 0.0f;
    //}
...
float pidGetErrorIntegral(int axis) {

	return (pidState[axis].errorGyroIf/pidState[axis].kI);
...
static void pidLevel(pidState_t *pidState, flight_dynamics_index_t axis, float horizonRateMagnitude)
{
...
#ifdef USE_BLACKBOX
    axisPID_angleTgt[axis] = angleTarget;
#endif
    ...
    cur_PID.inav_angle.Target[axis] = angleTarget;
    cur_PID.inav_angle.Error[axis] = angleErrorDeg;
    cur_PID.inav_angle.newData = 1;
#endif
}
...
static void pidApplyMulticopterRateController(pidState_t *pidState, flight_dynamics_index_t axis)
{
    ...
    if (axis == FD_YAW) {
    	pidState->errorGyroIf = constrainf(pidState->errorGyroIf, -systemConfig()->cx_yaw_rate_Iterm_limit, systemConfig()->cx_yaw_rate_Iterm_limit);
    }
    ...
    axisPID_Setpoint[axis] = pidState->rateTarget;
    axisPID_rate[axis] = pidState->gyroRate;
#endif

#if defined(USE_TELEMETRY_XLOGGER)
    cur_PID.inav_rate.Rate[axis] = pidState->gyroRate;
    cur_PID.inav_rate.Target[axis] = pidState->rateTarget;
    cur_PID.inav_rate.Error[axis] = rateError;
    cur_PID.inav_rate.Pterm[axis] = newPTerm;
    cur_PID.inav_rate.Iterm[axis] = pidState->errorGyroIf;
    cur_PID.inav_rate.Dterm[axis] = newDTerm;
    cur_PID.inav_rate.PIDres[axis] = newOutputLimited;
    cur_PID.inav_rate.newData = 1;
    cur_PID.inav_angle.newData = 1;
#endif
}
...
#else
        desired_pitch_rate = pidState[FD_PITCH].rateTarget;
        pidApplyMulticopterRateController(&pidState[axis], axis);
#endif
    }
}
