...
uint8_t servos_check_pitch = 0;
uint8_t servos_check_payload = 0;

uint8_t servos_block = 0;
...
uint8_t servoRuleCount = 0;
static servoMixer_t currentServoMixer[MAX_SERVO_RULES];
int servoOutputEnabled;
...
int16_t stab_roll_value;
int16_t stab_pitch_value;
...
void servoMixer(float dT)
{
    ...
    //input[INPUT_PAYLOAD]     = (int)(payload[0] * 500.0);
    


    float sc = systemConfig()->cx_payload_B / 10.0;
    float freq = millis() / sc;
    int a = ((int)(freq)) % 2000 - 1000;
    a = abs(a) - 500;
    
    if( systemConfig()->cx_payload_always_on > 0 ){
        input[INPUT_PAYLOAD]     = a;
        input[INPUT_PAYLOAD] = (int)(input[INPUT_PAYLOAD] / 100.0 * systemConfig()->cx_payload_A); 
    }  else {
        input[INPUT_PAYLOAD] = (int)(payload[0] * 500.0 / 100.0 * systemConfig()->cx_payload_A);
    }
    


		if( servos_check_pitch == 1 ){
			input[INPUT_STABILIZED_PITCH] = 500;
		}
		if( servos_check_pitch == 2 ){
			input[INPUT_STABILIZED_PITCH] = -500;
		}
    ...
    for (int i = 0; i < servoRuleCount; i++) {
        ...
        //overwrite pitch servo
        if( (servos_block == 1)&&( from == INPUT_STABILIZED_PITCH ) ){
        	servo[target] = systemConfig()->cx_servo_parking_value;
        }
    }

    stab_roll_value = input[INPUT_STABILIZED_ROLL];
    stab_pitch_value = input[INPUT_STABILIZED_PITCH];

    //6 channel
    //servo[5] = input[INPUT_PAYLOAD];
...
servoMixer_t getCurrentServoMixer(uint8_t rule) {
	return currentServoMixer[rule];
}
#endif
