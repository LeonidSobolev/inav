...
uint8_t cx_trust_gps_azumuth = 0;
...
uint8_t disable_mag = 0;
uint8_t disable_gps_az = 0;
...
static void imuMahonyAHRSupdate(float dt, const fpVector3_t * gyroBF, const fpVector3_t * accBF, const fpVector3_t * magBF, bool useCOG, float courseOverGround)
{
    ...
    if (magBF || useCOG) {
        ...
        //if (magBF && (vectorNormSquared(magBF) > 0.01f) && (disable_mag == 0) ) {
            fpVector3_t vMag;
...
{
#if defined(USE_MAG)
    const bool canUseMAG = sensors(SENSOR_MAG) && compassIsHealthy() && (disable_mag == 0);
#else
    ...
    bool useMag = false;
    bool useCOG = false;
    bool cx_trust_cog = false;
    
    uint16_t min_as_for_comp_az = systemConfig()->cx_min_as_for_comp_az;
    uint16_t min_gs_for_comp_az = systemConfig()->cx_min_gs_for_comp_az;

        //in this option we use GPS, because we can plane
    if( (systemConfig()->cx_land_type >= 3)  || STATE(FIXED_WING) ) {
        bool canUseCOG = sensors(SENSOR_GPS) && STATE(GPS_FIX) && (gpsSol.numSat >= 6) && (gpsSol.groundSpeed >= min_gs_for_comp_az) && (disable_gps_az == 0);
            ...
            if (gpsHeadingInitialized) {
                ...
                if (ABS(gpsSol.groundCourse - attitude.values.yaw) < DEGREES_TO_DECIDEGREES(MAX_GPS_HEADING_ERROR_DEG) || !canUseMAG) {
                    ...
                    if (sensors(SENSOR_PITOT)) {
                        if( pitot.airSpeed >= min_as_for_comp_az ){
                        	cx_trust_cog = true;
                        }
                    }
                }
    ...
    cx_trust_gps_azumuth = cx_trust_cog;
}
