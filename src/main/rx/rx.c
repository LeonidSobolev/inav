...
// FIXAR improvement: check receiver status and log it
uint8_t ics_checkRX_oldval = 0;

void ics_checkRX_func(void) {
	uint8_t ics_checkRX = 	(rxIsReceivingSignal() 		<< 0) |
							(rxAreFlightChannelsValid()	<< 1);
#if defined(USE_TELEMETRY_XLOGGER)
	if(ics_checkRX != ics_checkRX_oldval) {
		cur_Flags.INAV_flags.receiver_status = ics_checkRX;
		cur_Flags.INAV_flags.curTime = millis();
		cur_Flags.INAV_flags.newData = 1;
	}
#endif
	ics_checkRX_oldval = ics_checkRX;
}
