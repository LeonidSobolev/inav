...
bool sbusInit(const rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig)
{
    ...
    static sbusFrameData_t sbusFrameData;
    	...
    	//determine the channel, start and stop of functions
	FailOffAuxChannel = 0; FailOffAuxStart = 0; FailOffAuxStop = 0;
	ArmAuxChannel = 0; ArmAuxStart = 0; ArmAuxStop = 0;
	PosHoldAuxChannel = 0; PosHoldAuxStart = 0; PosHoldAuxStop = 0;
	AltHoldAuxChannel = 0; AltHoldAuxStart = 0; AltHoldAuxStop = 0;
	AngleAuxChannel = 0; AngleAuxStart = 0; AngleAuxStop = 0;
	NavWPAuxChannel = 0; NavWPAuxStart = 0; NavWPAuxStop = 0;
	for (int i=0;i<MAX_MODE_ACTIVATION_CONDITION_COUNT;i++){
		const modeActivationCondition_t *mac = modeActivationConditions(i);
		switch (mac->modeId){
			case BOXFAILOFF:
				FailOffAuxChannel = mac->auxChannelIndex + NON_AUX_CHANNEL_COUNT ;
				FailOffAuxStart = MODE_STEP_TO_CHANNEL_VALUE(mac->range.startStep);
				FailOffAuxStop = MODE_STEP_TO_CHANNEL_VALUE(mac->range.endStep);
			break;
			case BOXARM:
				ArmAuxChannel = mac->auxChannelIndex + NON_AUX_CHANNEL_COUNT ;
				ArmAuxStart = MODE_STEP_TO_CHANNEL_VALUE(mac->range.startStep);
				ArmAuxStop = MODE_STEP_TO_CHANNEL_VALUE(mac->range.endStep);
			break;
			case BOXNAVPOSHOLD:
				PosHoldAuxChannel = mac->auxChannelIndex + NON_AUX_CHANNEL_COUNT ;
				PosHoldAuxStart = MODE_STEP_TO_CHANNEL_VALUE(mac->range.startStep);
				PosHoldAuxStop = MODE_STEP_TO_CHANNEL_VALUE(mac->range.endStep);
			break;
			case BOXNAVALTHOLD:
				AltHoldAuxChannel = mac->auxChannelIndex + NON_AUX_CHANNEL_COUNT ;
				AltHoldAuxStart = MODE_STEP_TO_CHANNEL_VALUE(mac->range.startStep);
				AltHoldAuxStop = MODE_STEP_TO_CHANNEL_VALUE(mac->range.endStep);
			break;
			case BOXANGLE:
				AngleAuxChannel = mac->auxChannelIndex + NON_AUX_CHANNEL_COUNT ;
				AngleAuxStart = MODE_STEP_TO_CHANNEL_VALUE(mac->range.startStep);
				AngleAuxStop = MODE_STEP_TO_CHANNEL_VALUE(mac->range.endStep);
			break;
			case BOXNAVWP:
				NavWPAuxChannel = mac->auxChannelIndex + NON_AUX_CHANNEL_COUNT ;
				NavWPAuxStart = MODE_STEP_TO_CHANNEL_VALUE(mac->range.startStep);
				NavWPAuxStop = MODE_STEP_TO_CHANNEL_VALUE(mac->range.endStep);
			break;
			default:
			break;
		}
