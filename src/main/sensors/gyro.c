...
int16_t old_gyro_val;
timeDelta_t gyro_lag_time;
void cx_check_gyro(timeDelta_t gyroUpdateDeltaUs){
    if( systemConfig()->cx_max_gyro_lag_time_us > 10 ){
		if( gyroADC[Y] == old_gyro_val ){
			gyro_lag_time += gyroUpdateDeltaUs;
			if( gyro_lag_time > systemConfig()->cx_max_gyro_lag_time_us ){
				gyro_lag_flag = 1;
			}
		} else{
			old_gyro_val = gyroADC[Y];
			gyro_lag_time = 0;
		}
    }
}
...
{
	// FIXAR: raw data to xlogger
	for (int ax = 0; ax< XYZ_AXIS_COUNT; ax++){
		cur_xlog_sens.gyroADCRaw[ax] = gyroDev0.gyroADCRaw[ax];
	}
    ...
    if (gyroDev0.readFn(&gyroDev0)) {
        if (isCalibrationComplete(&gyroCalibration)) {
            ...
            cx_check_gyro(gyroUpdateDeltaUs);
