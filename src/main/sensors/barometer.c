...
uint32_t baro_time_interval = 0;
timeUs_t baro_time_last_calc = 0;
int32_t baro_alt_prev = 0;

int32_t minas = 9999;
int32_t maxas = 0;

int32_t baroCalculateAltitude(timeUs_t currentTimeUs)
{
    baro_time_interval = currentTimeUs - baro_time_last_calc;
    ...
    if (!baroIsCalibrationComplete()) {
        ...
        //If emul do not calc baro, just use external value
        ...
        //If emul or real, calc altSpeed always
        if( baro_time_interval > 10000 ){ //100hz
           baro.baroAltSpeed = 0.05 * (baro.BaroAlt - baro_alt_prev) / (baro_time_interval / 1000000.0) + 0.95 * baro.baroAltSpeed;
           if( minas > baro.baroAltSpeed ) minas = baro.baroAltSpeed;
           if( maxas < baro.baroAltSpeed ) maxas = baro.baroAltSpeed;
           baro_alt_prev = baro.BaroAlt; 
           baro_time_last_calc = currentTimeUs;   
        }           
...
int32_t baroGetLatestAltSpeed(void)
{
    return baro.baroAltSpeed;
}
