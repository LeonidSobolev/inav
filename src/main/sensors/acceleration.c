...
FASTRAM float cx_acc[XYZ_AXIS_COUNT];
...
#ifdef USE_ACC_ICM42605
    case ACC_ICM42605: // MPU6050
...
void accUpdate(void)
{
    ...
    for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
        cx_acc[axis] = (float)accADC[axis] / acc.dev.acc_1G;
        acc.accADCf[axis] = cx_acc[axis];
    }
