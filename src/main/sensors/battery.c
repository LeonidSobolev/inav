...
static uint32_t batteryInitialSpents = 0;
...
{
	uint32_t scaler = (1 * ADCVREF * 1000) / 0xFFF;
    int32_t microvolts = ((uint32_t)src * scaler) - (int32_t)batteryConfig()->current.offset * 1000;
...
uint32_t calculateInitialCapacity() {

		float v = ((float)getBatteryAverageCellVoltage())/100;
	    float x0, x1, y0, y1;
	    if (v< systemConfig()->cx_ptsV_4) {
	        x0 = systemConfig()->cx_ptsV_5;
	        x1 = systemConfig()->cx_ptsV_4;
	        y0 = systemConfig()->cx_ptsC_5;
	        y1 = systemConfig()->cx_ptsC_4;
	    }
	    else if (	(systemConfig()->cx_ptsV_4 <= v) && (v < systemConfig()->cx_ptsV_3)	) {
	        x0 = systemConfig()->cx_ptsV_4;
	        x1 = systemConfig()->cx_ptsV_3;
	        y0 = systemConfig()->cx_ptsC_4;
	        y1 = systemConfig()->cx_ptsC_3;
	    }
	    else if (	(systemConfig()->cx_ptsV_3 <= v) && (v < systemConfig()->cx_ptsV_2)	) {
	        x0 = systemConfig()->cx_ptsV_3;
	        x1 = systemConfig()->cx_ptsV_2;
	        y0 = systemConfig()->cx_ptsC_3;
	        y1 = systemConfig()->cx_ptsC_2;
	    }
	    else if (	(systemConfig()->cx_ptsV_1 <= v) && (v < systemConfig()->cx_ptsV_2)	) {
	        x0 = systemConfig()->cx_ptsV_2;
	        x1 = systemConfig()->cx_ptsV_1;
	        y0 = systemConfig()->cx_ptsC_2;
	        y1 = systemConfig()->cx_ptsC_1;
	    }
	    else {
	        x0 = systemConfig()->cx_ptsV_1;
	        x1 = systemConfig()->cx_ptsV_0;
	        y0 = systemConfig()->cx_ptsC_1;
	        y1 = systemConfig()->cx_ptsC_0;
	    }
...
float voltage_launch_time = 0;
float voltage_empty_time = 0;

float get_voltage_launch_time(void) {
	return voltage_launch_time;
}

void ics_clearVoltageFilters(void) {
    voltage_launch_time = 0;
    voltage_empty_time = 0;
}

void ics_updateBattVoltageMonitor(uint32_t vbatTimeDelta) {

	// vbatTimeDelta -> us

	float dt = ((float) vbatTimeDelta)*1e-6;		//dt -> s
	float cur_v = getBatteryVoltage() / 1e2;	// cur_v -> V


	if( systemConfig()->cx_batt_filter_time > 0.0001 ){
		if( cur_v < systemConfig()->cx_batt_launch_min_value ){
			voltage_launch_time += dt;
		} else {
			voltage_launch_time = 0;
		}
		if( cur_v < systemConfig()->cx_batt_empty_value ){
			voltage_empty_time += dt;
		} else {
			voltage_empty_time = 0;
		}
	}
...
void batteryUpdate(uint32_t vbatTimeDelta)
{
    ...
    if (batteryState == BATTERY_NOT_PRESENT && vbat > VBATT_PRESENT_THRESHOLD)
    {
        ...
        if (cells > 6) cells = 6; // something is wrong, we expect 6 cells maximum (and autodetection will be problematic at 6+ cells)
    ...
    if (batteryState != BATTERY_NOT_PRESENT) {
    	...
    	ics_updateBattVoltageMonitor(vbatTimeDelta);
        ...
        if ((batteryConfig()->capacity.value > 0)) {
            ...
            batteryRemainingCapacity = (drawn > (int32_t)(capacityDiffBetweenFullAndEmpty- batteryInitialSpents) ? 0 : (capacityDiffBetweenFullAndEmpty - drawn - batteryInitialSpents));
        }


        if (batteryUseCapacityThresholds) {
            if ( (batteryRemainingCapacity == 0) || (((float)voltage_empty_time) > systemConfig()->cx_batt_filter_time) )
            	// Beliaev: add voltage criteria
...
uint8_t calculateBatteryPercentage(void)
{
    ...
    if (feature(FEATURE_CURRENT_METER) && (batteryConfig()->capacity.value > 0) && (batteryConfig()->capacity.critical > 0)) {
        uint32_t capacityDiffBetweenFullAndEmpty = batteryConfig()->capacity.value - batteryConfig()->capacity.critical;
